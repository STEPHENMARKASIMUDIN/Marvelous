...
export class TrainYourDragon extends Component {
    ...
    async handleRequest() {
        const response = await GetAxios('/How to Train Your Dragon/Movies', '215')
        this.setState({
            ...this.state,
            movies: response
        })
                    ...
                    <TableBody>
                        {this.state.movies ? this.state.movies.map(row => (
                            <TableRow key={row.time}>
                                <TableCell><ListItem button onClick={() => { this.handleClick(`${'/How to Train Your Dragon/Movies/'}${row.name}`, row.name, 1) }}>{row.name}</ListItem></TableCell>
                            </TableRow>
                        )) : <Typography component="h2" variant="h6" color="primary" gutterBottom>
                                Loading...
                            </Typography>}
