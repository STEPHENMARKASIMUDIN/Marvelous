...
export class Others extends Component {
    ...
    async handleRequest() {
        const response = await GetAxios('/Others/Movies', '215')
        this.setState({
            ...this.state,
            movies: response
        })
        ...
        return (
            ...
            <Fragment>
                ...
                <form id="searchOthers">
                    <input type="text" placeholder="Search movies..." style={{ height: '30px', width: '290px', padding: '10px' }} />
                </form>
                <Table size="small" >
                    <TableHead>
                        <TableRow>
                    ...
                    <TableBody>
                        {this.state.movies ? this.state.movies.map(row => (
                            <TableRow key={row.time}>
                                <TableCell><ListItem button onClick={() => { this.handleClick(`${'/Others/Movies/'}${row.name}`, row.name, 1) }}>{row.name}</ListItem></TableCell>
                            </TableRow>
                        )) : <Typography component="h2" variant="h6" color="primary" gutterBottom>
                                Loading...
                            </Typography>}
